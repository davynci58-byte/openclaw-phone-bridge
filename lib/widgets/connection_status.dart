...
/// Connection status bar at the top of the home screen.
    ...
    return ValueListenableBuilder<NodeConnectionState>(
      ...
      builder: (context, state, _) {
        final (color, icon, label) = switch (state) {
          NodeConnectionState.connected => (Colors.green, Icons.cloud_done, 'OpenClaw Connected'),
          NodeConnectionState.connecting => (Colors.orange, Icons.cloud_upload, 'Connecting...'),
          NodeConnectionState.reconnecting => (Colors.orangeAccent, Icons.sync, 'Reconnecting...'),
          NodeConnectionState.error => (Colors.red, Icons.cloud_off, 'Connection Error'),
          NodeConnectionState.disconnected => (Colors.grey, Icons.cloud_off, 'Disconnected'),
        };
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                Text(service.shortDeviceId, style: TextStyle(fontSize: 10, color: color.withValues(alpha: 0.7), fontFamily: 'monospace')),
              ],
            ),
          ),
        );
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: ValueListenableBuilder<String>(
          ...
          builder: (_, status, __) => Column(
            ...
            children: [
              _row('Status', status),
              _row('Device', service.shortDeviceId),
              _row('Role', 'Node'),
...
/// Compact dot indicator for app bars.
